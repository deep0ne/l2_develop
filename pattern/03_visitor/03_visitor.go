/*
Паттерн посетитель относится к поведенческим паттернам
"Посетитель" позволяет добавлять новые операции к объектами, не изменяя эти объекты.
Создаётся интерфейс посетителя, который содержит методы для каждого типа объекта, которые он может посетить.
Он позволяет избежать изменений структур (классов), но усложняет код программы.
